...
export default function Chat(params: { chatid: string, scenarioId: string, scenario: any,character:any }) {
  ...
  const { messages, input, handleInputChange, handleSubmit } = useChat({
    api: '/api/chat/' + params.scenarioId,
    headers: { 'X-ChatId': params.chatid },
    onFinish(messages) {
      setHint('')
      currentMessage = messages.content
      synthesizeSpeechWithVoice(messages.content, params.character.voice,audioData => {
        if (audioData) {
  ...
  useEffect(() => {
    ...
    synthesizeSpeechWithVoice(params.scenario.welcomeMessage, params.character.voice,audioData => {
      if (audioData) {
        handleAudioPlay(audioData)
      } else {
        console.error('Speech synthesis failed or returned no audio');
              ...
              <div className='w-full flex flex-col-reverse gap-4'>
                ...
                <div className='relative w-full flex flex-row justify-center items-end gap-6'>
                  ...
                  <div className='flex flex-row gap-4 items-end'>
                    <Button
                      type='button'
                      size={'icon'}
                      className={`h-fit p-4 bg-[#42C83C] w-fit rounded-full shadow-[0.25rem_0.25rem_0_#1E6AA7] ${loading == true ? 'animate-pulse' : ''}`}
                ...
                {hint.length > 0 ?
                  <div className='flex flex-col gap-4'>
                    <div className='text-xl w-full text-center animate-custom-bounce rounded-full bg-[#42C83C] text-white py-2 px-4 w-fit font-bold text-xl shadow-[0.25rem_0.25rem_0_#1E6AA7]e'>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {(100 * (Math.pow((totalFluencyScore+totalAccuracyScore) / dialogLength / 200, 1))).toFixed(1)}
              </div>
              </div>

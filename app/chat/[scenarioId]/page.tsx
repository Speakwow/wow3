...
export default async function ChatPage({ params }: { params: { scenarioId: string } }) {
  ...
  // const welcomeMessage = await kv.hget(`scenario@`+params.scenario,'welcomeMessage') as string
  const scenario = await kv.hgetall(`scenario@` + params.scenarioId) as any
  let character;
  if (scenario.character) {
    character = await kv.hgetall('character@' + scenario.character) as any
  } else {
    character = await kv.hgetall('character@lkxxZbHMS932Cem1') as any
  }
    ...
    <div style={bgImage} className="h-screen bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90%" >
      <div className="mx-auto h-screen m p-8 flex flex-col items-center justify-center">
        ...
        {scenario.welcomeMessage ?
          <Initial chatid={chatid} scenarioId={params.scenarioId} scenario={scenario} character={character} />
          :
          <div>
            No Scenario
          </div>

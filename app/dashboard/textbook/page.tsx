...
export default async function Home() {
    ...
    const [rawTextbooks, currentBookId] = await Promise.all([mongo.db(DB_CORE).collection('textbooks').find({ access: 'public' }).toArray(), kv.hget('org:'+orgId, 'textbook')])
            ...
            <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8 bg-muted h-full">
                ...
                <ScrollArea className="w-full h-full flex flex-col gap-8">
                    <div className="w-full h-full flex flex-col gap-8 mb-24">
                        {textbooks.length > 0 ?
                            <TextbookSelector textbooks={textbooks} orgId={orgId} currentBookId={currentBookId as string} />
                            :
                            <div>
                                暂无可用课程
                            </div>

...
export default async function ChatPage({ params }: { params: { scenarioId: string } }) {
  ...
  const scenarioDoc = await getImgtalkById(params.scenarioId) as any
  ...
  const chatid = await createImgtalkRecord(userId as string)

  return (
    <div className="relative h-screen bg-muted p-4">

      <Card style={bgImage} className="relative w-full h-full rounded-[20px]" >

          {/* <div className="relative w-full h-fit rounded-[20px] overflow-hidden py-20">
            <Image className="" fill={true} objectFit="cover" alt='Image' src={scenario.referenceImage} />
          </div> */}

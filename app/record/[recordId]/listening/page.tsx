...
import { getChineseName, getFullName } from "@/lib/tools";
            ...
            <ScrollArea className="w-full h-full mb-4">
                <div className="p-4 h-full flex flex-col mb-10">
                    ...
                    <div className=" grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-10">
                        ...
                        <Card className="w-full col-span-1 h-fit pb-2 border-primary">
                            <CardHeader className="p-4 pb-0">
                        ...
                        <Card className="w-full col-span-full  h-fit pb-2">
                            <CardHeader className="p-4 pb-0">
                                <CardTitle className="text-md ">听力材料</CardTitle>
                                <CardDescription className="text-xs">
                                    请仔细聆听录音，并回答以下问题。
                                </CardDescription>
                            </CardHeader>
                    ...
                    <div className="pt-6 h-screen w-full">
                        <Card className="h-fit w-full">
                            ...
                            <Table>
                                ...
                                <TableBody>
                                    {record?.stu_answers.map((answer: string, index: number) => (
                                        <TableRow key={index}>
                                            <TableCell className="text-md max-w-64 ">{info?.questions[index]}</TableCell>
                                            <TableCell className="text-md max-w-64 text-muted-foreground">{answer}</TableCell>
                                            <TableCell className={`text-lg font-bold text-primary ${record?.report[index] ? "text-green-500" : "text-red-500"}`}>{record?.report[index] ? "正确" : "错误"}</TableCell>                                       
                                            <TableCell className="text-md max-w-64 text-muted-foreground">{info?.model_answer[index]}</TableCell>
                                        </TableRow>
                                    ))}
                                </TableBody>
                            </Table>

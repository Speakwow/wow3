...
import { finishRepeatRecord, getRepeatPageByIndex } from '@/lib/action/mongoIO'
...
export default async function Repeat({ params }: { params: { threadId: string, index: string,recordId:string,lessonId:string } }) {

    const section = await getRepeatPageByIndex(params.threadId,+params.index)
    if (!section) {
        const {final_score,current,duration} = await finishRepeatRecord(params.recordId)
    ...
    return (
        ...
        style={{ backgroundImage: `url('${section.image_url}')`,backgroundSize: 'cover'}}>
            ...
            <div className='z-50 mt-20 h-full  max-w-96 gap-6'>
                <RepeatText text={section.text} index={section.index} recordId={params.recordId} />
            </div>
            </div>
        </div>
